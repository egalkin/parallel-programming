...
void do_map(struct array_d m1, struct array_d m2) {
    ...
    #pragma omp parallel for default(none) shared(m1) schedule(static, CHUNK_SIZE)
    for (int i = 0; i < m1.size; ++i) {
    ...
    #pragma omp parallel for default(none) shared(temp_arr, m2) schedule(static, CHUNK_SIZE)
    for (int i = 0; i < m2.size; ++i) {
...
double do_reduce(struct array_d m) {
    ...
    #pragma omp parallel for default(none) shared(m, min_element) reduction(+:sinus_sum) schedule(static, CHUNK_SIZE)
